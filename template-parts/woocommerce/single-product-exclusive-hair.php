<?php
/**
 * Single Product - Exclusive Hair layout.
 *
 * Uses a fixed lot price plus product-form surcharge selector.
 */
...
$nh_exclusive_child_term = null;

foreach ($nh_product_terms as $nh_term) {
    if (! $nh_term instanceof WP_Term || ! $nh_exclusive_term instanceof WP_Term) {
        continue;
    }

    if ((int) $nh_term->parent === (int) $nh_exclusive_term->term_id) {
        $nh_exclusive_child_term = $nh_term;
        break;
    }

    $nh_ancestors = array_map('intval', get_ancestors($nh_term->term_id, 'product_cat', 'taxonomy'));

    if (in_array((int) $nh_exclusive_term->term_id, $nh_ancestors, true)) {
        $nh_exclusive_child_term = $nh_term;
        break;
    }
}

$nh_exclusive_url = $nh_exclusive_term instanceof WP_Term ? get_term_link($nh_exclusive_term) : '';
$nh_exclusive_child_url = $nh_exclusive_child_term instanceof WP_Term ? get_term_link($nh_exclusive_child_term) : '';

if (is_wp_error($nh_exclusive_url)) {
    $nh_exclusive_url = '';
}

if (is_wp_error($nh_exclusive_child_url)) {
    $nh_exclusive_child_url = '';
}
...
$nh_pricing = function_exists('nice_hair_get_exclusive_product_form_pricing')
    ? nice_hair_get_exclusive_product_form_pricing($product)
    : [
        'fixed_weight'    => null,
        'options'         => [],
        'default_key'     => '',
...
$nh_product_form_options = isset($nh_pricing['options']) && is_array($nh_pricing['options'])
    ? $nh_pricing['options']
    : [];
$nh_selected_option = isset($nh_pricing['selected_option']) && is_array($nh_pricing['selected_option'])
    ? $nh_pricing['selected_option']
    : null;
$nh_selected_form_key = is_array($nh_selected_option) ? (string) ($nh_selected_option['key'] ?? '') : '';
$nh_selected_price_html = is_array($nh_selected_option)
    ? (string) ($nh_selected_option['price_html'] ?? '')
    : '';
$nh_selected_sample = is_array($nh_selected_option['sample_image'] ?? null)
    ? $nh_selected_option['sample_image']
    : ['url' => '', 'alt' => ''];
$nh_selected_support_copy = is_array($nh_selected_option)
    ? (string) ($nh_selected_option['support_copy'] ?? '')
    : '';
$nh_has_complete_pricing = (bool) ($nh_pricing['is_complete'] ?? false);
...
$nh_selected_option_available = is_array($nh_selected_option) && ! empty($nh_selected_option['is_available']);
$nh_can_purchase = $product->is_purchasable() && $nh_is_in_stock && $nh_has_complete_pricing && $nh_selected_option_available;
$nh_purchase_notice = '';

if (! $nh_is_in_stock) {
    $nh_purchase_notice = $nh_is_unique
        ? __('Sold', 'nice-hair')
        : __('Out of stock', 'nice-hair');
} elseif (! $nh_has_complete_pricing || ! $nh_selected_option_available) {
    $nh_purchase_notice = __('Exclusive Hair pricing is incomplete for this item. Please contact us for assistance.', 'nice-hair');
}
?>

<section id="product-<?php the_ID(); ?>" <?php wc_product_class('nh-single-product nh-single-product--exclusive-hair'); ?>>
    <div class="nh-single-product__container">
        <div class="nh-single-product__top">
            <nav class="nh-single-product__breadcrumb" aria-label="Breadcrumb">
                ...
                <?php if ($nh_exclusive_url !== '') : ?>
                    <a href="<?php echo esc_url($nh_exclusive_url); ?>">[Exclusive hair]</a> -
                <?php endif; ?>
                <?php if ($nh_exclusive_child_term instanceof WP_Term && $nh_exclusive_child_url !== '') : ?>
                    <a href="<?php echo esc_url($nh_exclusive_child_url); ?>">[<?php echo esc_html($nh_exclusive_child_term->name); ?>]</a> -
                <?php endif; ?>
        ...
        <div class="nh-single-product__columns">
            ...
            <div class="nh-single-product__info">
                ...
                <?php if ($nh_product_form_options !== []) : ?>
                    <div class="nh-single-product__option-group nh-single-product__option-group--exclusive">
                        <span class="nh-single-product__option-label">[You can order any type of product from this bundle:]</span>

                        <div class="nh-single-product__product-form-shell">
                            <div class="nh-single-product__product-form-panel">
                                <div class="nh-single-product__product-form-list" data-nh-product-form-options>
                                    <?php foreach ($nh_product_form_options as $nh_option) : ?>
                                        <?php
                                        $nh_option_key = (string) ($nh_option['key'] ?? '');
                                        $nh_option_label = (string) ($nh_option['label'] ?? '');
                                        $nh_is_option_available = ! empty($nh_option['is_available']);
                                        $nh_option_sample = is_array($nh_option['sample_image'] ?? null)
                                            ? $nh_option['sample_image']
                                            : ['url' => '', 'alt' => ''];
                                        $nh_option_sample_url = (string) ($nh_option_sample['url'] ?? '');
                                        $nh_option_sample_alt = (string) ($nh_option_sample['alt'] ?? ($nh_option_label . ' sample'));
                                        $nh_option_support_copy = (string) ($nh_option['support_copy'] ?? '');
                                        ?>
                                        <button
                                            type="button"
                                            class="nh-single-product__product-form-option<?php echo $nh_selected_form_key === $nh_option_key ? ' is-active' : ''; ?><?php echo ! $nh_is_option_available ? ' is-disabled' : ''; ?>"
                                            data-nh-product-form-option
                                            data-form-key="<?php echo esc_attr($nh_option_key); ?>"
                                            data-price-html="<?php echo esc_attr((string) ($nh_option['price_html'] ?? '')); ?>"
                                            data-available="<?php echo $nh_is_option_available ? '1' : '0'; ?>"
                                            data-sample-src="<?php echo esc_attr($nh_option_sample_url); ?>"
                                            data-sample-alt="<?php echo esc_attr($nh_option_sample_alt); ?>"
                                            data-support-copy="<?php echo esc_attr($nh_option_support_copy); ?>"
                                            aria-pressed="<?php echo $nh_selected_form_key === $nh_option_key ? 'true' : 'false'; ?>"
                                            <?php disabled(! $nh_is_option_available); ?>>
                                            <span class="nh-single-product__product-form-thumb">
                                                <?php if ($nh_option_sample_url !== '') : ?>
                                                    <img src="<?php echo esc_url($nh_option_sample_url); ?>"
                                                         alt="<?php echo esc_attr($nh_option_sample_alt); ?>"
                                                         loading="lazy">
                                                <?php else : ?>
                                                    <span class="nh-single-product__product-form-thumb-fallback">
                                                        <?php echo esc_html(function_exists('mb_substr') ? mb_substr($nh_option_label, 0, 1) : substr($nh_option_label, 0, 1)); ?>
                                                    </span>
                                                <?php endif; ?>
                                            </span>
                                            <span class="nh-single-product__product-form-name"><?php echo esc_html($nh_option_label); ?></span>
                                        </button>
                                    <?php endforeach; ?>
                                </div>
                            ...
                            <aside class="nh-single-product__product-form-preview">
                                <div class="nh-single-product__product-form-preview-media" data-nh-product-form-preview-media<?php echo ($nh_selected_sample['url'] ?? '') === '' ? ' hidden' : ''; ?>>
                                    <img
                                        data-nh-product-form-preview-image
                                        src="<?php echo esc_url((string) (($nh_selected_sample['url'] ?? '') !== '' ? $nh_selected_sample['url'] : wc_placeholder_img_src('woocommerce_single'))); ?>"
                                        alt="<?php echo esc_attr((string) ($nh_selected_sample['alt'] ?? '')); ?>"
                                        loading="lazy"
                                    >
                                </div>

                                <p class="nh-single-product__product-form-preview-note">
                                    <span class="nh-single-product__product-form-preview-note-mark">(!)</span><?php esc_html_e('The image shows a sample of the product type, ', 'nice-hair'); ?><span class="nh-single-product__product-form-preview-note-strong"><?php esc_html_e('not the exact item made from this bundle.', 'nice-hair'); ?></span>
                                </p>
                            </aside>
                        </div>
                    </div>
                ...
                <div class="nh-single-product__purchase">
                    ...
                    <p class="nh-single-product__price-note" data-nh-product-form-support-copy<?php echo $nh_selected_support_copy === '' ? ' hidden' : ''; ?>>
                        <?php echo esc_html($nh_selected_support_copy); ?>
                    </p>
                    ...
                    <form class="nh-single-product__cart-form"
                        ...
                        <input type="hidden" name="nh_exclusive_product_form" value="<?php echo esc_attr($nh_selected_form_key); ?>" data-nh-product-form>
